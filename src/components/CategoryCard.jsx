...
export default function FashionCategories() {
  ...
  return (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6 max-w-screen-xl mx-auto">
      {topCategories.map((category, index) => (
        ...
        >
          <div 
            className="w-full h-64 sm:h-80 lg:h-96 flex items-center justify-center bg-cover bg-center bg-no-repeat transition-transform duration-300 group-hover:scale-110"
            style={{ backgroundImage: `url(${category.img})` }}
          >
            <div className="absolute inset-0 bg-black bg-opacity-40 group-hover:bg-opacity-50 transition-all duration-300"></div>
            <div className="relative flex flex-col items-center justify-center text-white px-6 py-4">
              <h3 className="text-lg sm:text-2xl font-semibold mb-2 text-center transition-all duration-300 group-hover:text-yellow-400">
                {category.title}
              </h3>
              <p className="text-sm sm:text-lg font-light text-center transition-all duration-300 group-hover:text-white">
                {category.rating} ⭐️
              </p>
            </div>
          </div>
